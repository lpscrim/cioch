...
export default function PerfectFitForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/perfectfit.html", {
        method: "POST",
      ...
      if (response.ok) {
        setSubmitMessage("Thank you! Your order has been sent.");
        form.reset();
        setTimeout(() => {
          setSubmitMessage("");
        }, 2000);
      ...
      setSubmitMessage(
        "Sorry, there was an error sending your order. Please try again."
      );
    } finally {
  ...
  return (
    ...
    >
      ...
      <input type="hidden" name="form-type" value="Perfect Fit" />
      ...
      {/* Example: submit message feedback */}
      {submitMessage && (
        <div
      ...
      <div className="mt-8 flex items-center justify-end gap-x-6">
        ...
        <button
          ...
          className={`cursor-pointer rounded-md bg-secondary px-4 py-2 text-sm font-semibold text-text shadow-xs focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent transition-all ${
            isSubmitting
